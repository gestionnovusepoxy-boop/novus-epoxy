...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // If quote has a booking, check for conflicts before confirming
  let booking: Record<string, unknown> | null = null;
  if (quote.booking_id) {
    const bookings = await query('SELECT * FROM bookings WHERE id = $1', [quote.booking_id]);
    if (bookings.length > 0) {
      booking = bookings[0] as Record<string, unknown>;
      ...
      if (conflicts.length > 0) {
        const bookedRows = await query(
          `SELECT jour1_date, jour1_slot, jour2_date, jour2_slot FROM bookings WHERE statut = 'confirme' AND (jour1_date >= CURRENT_DATE OR jour2_date >= CURRENT_DATE)`,
          []
        );
        ...
        return NextResponse.json({ success: false, conflict: true, message: 'Les dates choisies sont deja prises.', available_dates: available });
      }

      await query(`UPDATE bookings SET statut = 'confirme' WHERE id = $1`, [booking.id]);
    }
  }

  // Mark quote as deposit paid
  await query(
...
<h2 style="margin:0;font-size:20px;">Depot recu — Merci!</h2>
...
<p>Merci pour votre depot! Votre devis est maintenant <strong>confirme</strong>.</p>
${booking ? `<div style="background:#f0fdf4;border:1px solid #22c55e;border-radius:8px;padding:16px;margin:16px 0;">
...
<strong>Jason:</strong> 581-307-2678</p>
  ...
  if (clientTel) {
    ...
    const smsText = booking
      ...
      : `${prenom}, c'est Luca de Novus Epoxy! Depot bien recu, merci! Ton dossier est confirme. On te contacte sous peu pour planifier les travaux. Questions? 581-307-5983`;
    await sendSMS(clientTel, smsText).catch(() => {});
  }
